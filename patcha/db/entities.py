...
class EntityExtractor:
    ...
    def extract_entities_from_event(
        self, event: Union[Event, dict]
    ) -> EntityExtractionResult:
        ...
        try:
            ...
            all_entities = self._merge_entities(
                rule_based_entities, llm_result.entities
            )
            all_relationships = llm_result.relationships

            # Calculate overall confidence
            confidence = (
                len(rule_based_entities) * 0.8 + llm_result.confidence * 0.2
            ) / max(1, len(all_entities))
    ...
    def _extract_entities_rule_based(self, event: Union[Event, dict]) -> List[Entity]:
        ...
        capitalized_entities = self._extract_capitalized_entities(
        ...
        file_patterns = re.findall(
            r"\b\w+\.(py|js|ts|java|rs|go|cpp|c|h|md|json|yaml|yml|sql|html|css|scss)\b",
            text_content.lower(),
        )
    ...
    def _extract_capitalized_entities(
        ...
        for match in matches:
            ...
            if (
                len(match) < 3
                or match in common_words
                or match.lower() in self.tech_patterns
                or match.lower() in self.concept_patterns
            ):
                continue
            ...
            entity_id = (
                f"{entity_type.value}_{match.replace(' ', '_').replace('.', '_')}"
            )
            entities.append(
                Entity(
                    id=entity_id,
    ...
    def _classify_capitalized_entity(
        ...
        if any(
            ext in entity_lower
            for ext in [".py", ".js", ".md", ".txt", ".json", ".xml"]
        ):
    ...
    def _extract_entities_llm(
        ...
        try:
            ...
            result = self._parse_llm_response(
                response.choices[0].message.content, timestamp
            )
    ...
    def _parse_llm_response(
        ...
        try:
            ...
            for rel_data in data.get("relationships", []):
                ...
                if (
                    source_name in entity_name_to_id
                    and target_name in entity_name_to_id
                ):
