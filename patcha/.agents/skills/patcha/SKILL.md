---
name: patcha
description: Use patcha MCP tools to recall past user activity, recent changes, and work history before falling back to git or filesystem commands.
---

# patcha

patcha tracks the user's device activity — git commits, terminal commands, browser tabs, and editor sessions — and exposes it via three MCP tools.

## When to use patcha tools

Always call `mcp__patcha__search_activity` first when the user asks:
...
Only fall back to `git log`, `git diff`, or filesystem commands if patcha returns no relevant results.

## Tools

### `mcp__patcha__search_activity`
...
### `mcp__patcha__get_working_memory`
...
### `mcp__patcha__get_recent_activity`
