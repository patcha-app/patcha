...
class DataMigration:
    ...
    def migrate_date_range(
        ...
        """
        console.print(f"[bold cyan]Migrating data from {start_date} to {end_date}[/bold cyan]")
    ...
    def migrate_single_date(
        ...
        try:
            ...
            for i in range(0, len(events), batch_size):
                ...
                try:
                    batch_tasks = self.task_identifier.identify_tasks_from_activities(batch)
    ...
    def _convert_activity_to_event(self, activity_data: Dict[str, Any]) -> Optional[Event]:
    ...
    def _link_activities_to_task(self, task, activities_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            console.print(f"[yellow]Warning: Could not link activities to task {task.id}: {e}[/yellow]")
    ...
    def _display_migration_report(self, report: Dict[str, Any]):
        ...
        console.print(f"\n[bold]Summary:[/bold]")
        console.print(f"• Activities processed: {summary['total_activities_processed']}")
        ...
        if daily_stats:
            ...
            for i, (date_str, stats) in enumerate(sorted_dates[:5], 1):
                console.print(f"{i}. {date_str}: {stats.get('tasks_identified', 0)} tasks from {stats.get('activities_processed', 0)} activities")
    ...
    def analyze_migration_feasibility(
        ...
        console.print(f"[bold cyan]Analyzing migration feasibility for {start_date} to {end_date}[/bold cyan]")
        ...
        while current_date <= end_date:
            try:
                ...
                analysis["daily_activity_counts"][current_date.isoformat()] = activity_count
        ...
        console.print(f"• Total activities to process: {analysis['total_activities']:,}")
        console.print(f"• Days with data: {analysis['dates_with_data']}/{analysis['date_range']['total_days']}")
        console.print(f"• Estimated tasks to create: {analysis['estimated_tasks']:,}")
        console.print(f"• Average activities per day: {analysis['total_activities'] / max(analysis['dates_with_data'], 1):.1f}")
        ...
        if analysis["potential_issues"]:
            ...
            if len(analysis["potential_issues"]) > 5:
                console.print(f"• ... and {len(analysis['potential_issues']) - 5} more issues")
    ...
    def cleanup_duplicate_tasks(self, target_date: Optional[date] = None) -> Dict[str, Any]:
        ...
        if target_date:
            ...
            date_range = f"from last 30 days"
        ...
        for i, task1 in enumerate(tasks):
            for j, task2 in enumerate(tasks[i+1:], start=i+1):
                similarity_score = self._calculate_task_similarity_for_cleanup(task1, task2)
                ...
                if similarity_score > 0.9:  # Very high similarity threshold
                    ...
                    if (task1.activity_count > task2.activity_count or
                        (task1.activity_count == task2.activity_count and task1.confidence_score > task2.confidence_score)):
        ...
        console.print(f"[green]Cleanup completed: {removed_count} duplicate tasks removed[/green]")
    ...
    def _calculate_task_similarity_for_cleanup(self, task1, task2) -> float:
        ...
        if task1.title and task2.title:
            if task1.title.lower().strip() == task2.title.lower().strip():
                similarity += 0.5
            elif task1.title.lower() in task2.title.lower() or task2.title.lower() in task1.title.lower():
                similarity += 0.3
            factors += 0.5

        # Time proximity (tasks very close in time might be duplicates)
        if task1.start_time and task2.start_time:
            time_diff_minutes = abs((task1.start_time - task2.start_time).total_seconds()) / 60
