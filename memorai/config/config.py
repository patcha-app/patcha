...
load_dotenv(_DEFAULT_MEMORAI_DIR / ".env")  # user config — works in dev and binary
load_dotenv()                               # project .env fallback (dev only)


class Config(BaseModel):
    openai_api_key: str
    qdrant_url: str = "http://localhost:6333"
    qdrant_path: Path = _DEFAULT_MEMORAI_DIR / "qdrant_storage"
    data_dir: Path = _DEFAULT_MEMORAI_DIR / "data"
    log_level: str = "INFO"
    ...
    def from_env(cls) -> "Config":
        return cls(
            ...
            qdrant_path=Path(os.getenv("QDRANT_PATH", str(_DEFAULT_MEMORAI_DIR / "qdrant_storage"))),
            data_dir=Path(os.getenv("DATA_DIR", str(_DEFAULT_MEMORAI_DIR / "data"))),
